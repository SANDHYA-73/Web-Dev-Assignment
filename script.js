...
//     const endpoint = 'YOUR_GETFORM_ENDPOINT_HERE';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function calculateTotalFrames() {
    ...
    } else if (innerWidth > 575 && innerWidth <= 992) {
    ...
    return totalFramseOfElements;
  }

  function createIndicators(totalFramseOfElements) {
    scroll_indicator_container.innerHTML = ''; // Clear existing indicators

    for (let i = 0; i < totalFramseOfElements; i++) {
      const li = document.createElement("li");

      scroll_indicator_container.appendChild(li);
      li.classList.remove("active_service");
      if (i === 0) {
        li.classList.add("active_service");
      }
  ...
  function updateScroller() {
    const totalFramseOfElements = calculateTotalFrames();
    createIndicators(totalFramseOfElements);
  }
...
form.addEventListener("submit", (e)=>{
  ...
  console.log("data ", data)
    const url = "https://getform.io/f/arolydwb";
